...
import com.tn3112.nvt.youtubeaudioplayer.data.NetworkType;
...
public class ExoPlayerService extends Service {
        ...
        progressHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (exoPlayer != null && stateCode == Player.STATE_READY || stateCode == Player.STATE_BUFFERING) {
                    ...
                    bundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, Constants.PLAYBACK_PROGRESS_CHANGED);
                    bundle.putInt(Constants.EXTRA_TRACK_PROGRESS, trackCurrentPosition);
                    utils.sendLocalBroadcastMessage(Constants.ACTION_PLAYER_STATE_CHANGED, bundle);
                    ...
                    Bundle bundle = new Bundle();
                    bundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, Player.STATE_IDLE);
                    utils.sendLocalBroadcastMessage(Constants.ACTION_PLAYER_STATE_CHANGED, bundle);
                } else {
    ...
    private void registerReceivers() {
        ...
        receiverManager.registerLocalReceiver(playerCommandsBroadcastReceiver, new IntentFilter(Constants.ACTION_PLAYER_CHANGE_STATE));
    ...
    private void startNotificationService(YoutubeSongDto song) {
        if (!utils.isServiceRunning(PlayerNotificationService.class)) {
            ...
            Bundle bundle = new Bundle();
            bundle.putParcelable(Constants.EXTRA_SONG, song);
            intent.putExtra(Constants.EXTRA_SONG, bundle);
            startService(intent);
        }
    }
    ...
    public class PlayerStateListener implements Player.EventListener {
        ...
        public void onPlayerError(ExoPlaybackException error) {
            Bundle bundle = new Bundle();
            bundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, Constants.PLAYER_ERROR);
            switch (error.type) {
                case ExoPlaybackException.TYPE_SOURCE:
                    bundle.putSerializable(Constants.PLAYER_ERROR_THROWABLE, error.getSourceException());
                    Log.e(TAG, "TYPE_SOURCE: " + error.getSourceException().getMessage());
                    break;
                case ExoPlaybackException.TYPE_RENDERER:
                    bundle.putSerializable(Constants.PLAYER_ERROR_THROWABLE, error.getRendererException());
                    Log.e(TAG, "TYPE_RENDERER: " + error.getRendererException().getMessage());
                    break;
                case ExoPlaybackException.TYPE_UNEXPECTED:
                    bundle.putSerializable(Constants.PLAYER_ERROR_THROWABLE, error.getUnexpectedException());
        ...
        public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
            Bundle bundle = new Bundle();
            bundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, playbackState);
            switch (playbackState) {
                case Player.STATE_ENDED:
                    ...
                    utils.sendLocalBroadcastMessage(Constants.ACTION_PLAYER_STATE_CHANGED, playbackEndedBundle);
                    startIdlingRunnable();
                ...
                case Player.STATE_READY:
                    ...
                    bundle.putInt(Constants.EXTRA_TRACK_DURATION, (int) (exoPlayer.getDuration() / 1000));
                    bundle.putBoolean(Constants.EXTRA_PLAYBACK_STATUS, exoPlayer.getPlayWhenReady());
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                ...
                case PlayerAction.NEXT:
                    ...
                    nextBundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, PlayerAction.NEXT);
                    utils.sendLocalBroadcastMessage(Constants.ACTION_PLAYLIST_NAVIGATION, nextBundle);
                    break;
                case PlayerAction.BACK:
                    Bundle backBundle = new Bundle();
                    backBundle.putInt(Constants.EXTRA_PLAYER_STATE_CODE, PlayerAction.BACK);
                    utils.sendLocalBroadcastMessage(Constants.ACTION_PLAYLIST_NAVIGATION, backBundle);
                    break;
                case PlayerAction.CHANGE_PLAYBACK_PROGRESS:
                    int progress = intent.getIntExtra(Constants.EXTRA_TRACK_PROGRESS, 0);
